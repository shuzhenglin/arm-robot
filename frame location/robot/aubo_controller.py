import sys
sys.path.append(r'D:\Dot_Match\robot')
sys.path.append(r'D:\Dot_Match\robot\Dlls')

from robot.robotcontrol import *
import numpy as np
import logging
import yaml
import math
...
class AuboRobot():
    ...
    def robot_start(self):
        ...
        self.robot.set_end_max_line_acc(end_maxacc=0.5)
        self.robot.set_end_max_line_velc(end_maxvelc=0.5)
    ...
    def set_end_coord(self, relative_pos, relative_ori):
        end_coord = {'coord_type': 2,
                'calibrate_method': 0,
                'calibrate_points':
        ...
        res = self.robot.set_relative_offset_on_user(relative_pos=relative_pos, relative_ori=relative_ori, user_coord=end_coord)
        if res != RobotErrorType.RobotError_SUCC:
            raise AuboError("[Aubo-Error]: Robot set coord error")
        else:
            self.robot.set_user_coord(end_coord)
