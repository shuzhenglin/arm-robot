...
class Auboi5Robot:
    ...
    def connect(self, ip='localhost', port=8899):
        ...
        * NOTES:
        ...
        if self.rshd >= 0:
            if not self.connected: #上下文句柄RSHD>0,但connect是false
                if libpyauboi5.login(self.rshd, ip, port) == 0:#连接成功
                    self.connected = True
                    time.sleep(0.5)
                    return RobotErrorType.RobotError_SUCC#返回0
                else:
                    logger.error("login failed!")
                    return RobotErrorType.RobotError_LOGIN_FAILED#失败返回2005
            else:
                logger.info("already connected.")
                return RobotErrorType.RobotError_SUCC#connect为true
        else:
            logger.error("RSHD uninitialized!!!")
            return RobotErrorType.RobotError_RSHD_UNINIT#库未初始化
